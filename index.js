...
const pubsub = new PubSub({projectId: config.cloud.project_id});
...
exports.consumeEvent = (message, callback) => {
  ...
  // TODO
  msg = JSON.parse(JSON.stringify(config.slack.message_template));
  msg.text = 'A new event has been published'
  msg.attachments = [
    {
      color: 'good',
      footer: 'Slack | Google Drive',
      text: '```\n' + JSON.stringify(slackEvent) + '\n```',
      ts: +new Date()/1000
    }
  ];
